...
class EditCategoryDialog : DialogFragment() {
    ...
    override fun onCreateView(
        ...
        val category = Gson().fromJson(arguments?.getString(Constants.CATEGORY_KEY), FoodCategory::class.java)
        view.category_name.setText(category.name)
        view.category_description.setText(category.description)
        view.category_image.visibility = View.VISIBLE
        Picasso.get().load("${Routes.HOST_END_POINT}${category.image}").into(view.category_image)
            ...
            imageSelectDialog.setOnImageSelectorListener(object : ImageSelectorListener {
                ...
                override fun onSingleImageSelected(image: String) {
                    if(image != "") {
                        ...
                        } catch (e: Exception) { TingToast(context!!, e.localizedMessage, TingToastType.DEFAULT).showToast(Toast.LENGTH_LONG) }
                    } else { TingToast(context!!, "Image Cannot Be Null", TingToastType.DEFAULT).showToast(Toast.LENGTH_LONG) }
                }
        ...
        view.dialog_button_save.setOnClickListener {
            ...
            val url = "${Routes.updateCategory}${category.id}/"
