...
class MenuDishTableViewAdapter (val context: Context) : AbstractTableAdapter<ColumnHeaderModel?, RowHeaderModel?, CellModel?>() {

    private val menuDishTableViewModel: MenuDishTableViewModel = MenuDishTableViewModel()

    override fun onCreateCellViewHolder(parent: ViewGroup, viewType: Int): AbstractViewHolder {
        return when(viewType) {
            Constants.IMAGE_TABLE_VIEW_CELL -> {
                val layout = LayoutInflater.from(context).inflate(R.layout.tableview_cell_image_view, parent, false)
                ImageCellViewHolder(layout)
            }
            Constants.WAITER_TABLE_VIEW_CELL -> {
                val layout = LayoutInflater.from(context).inflate(R.layout.tableview_cell_table_waiter_view, parent, false)
                TableWaiterCellViewHolder(layout)
            }
            Constants.ACTIONS_TABLE_VIEW_CELL -> {
                val layout = LayoutInflater.from(context).inflate(R.layout.tableview_actions_cell_view, parent, false)
    ...
    override fun onBindCellViewHolder(
        ...
        when (holder) {
            is CellViewHolder -> { (holder as CellViewHolder).setCellModel(cell!!, columnPosition) }
            is ImageCellViewHolder -> { (holder as ImageCellViewHolder).setCellModel(cell!!, columnPosition) }
            is TableWaiterCellViewHolder -> { (holder as TableWaiterCellViewHolder).setCellModel(cell!!, columnPosition, 2) }
            is ActionsCellViewHolder -> { (holder as ActionsCellViewHolder).setCellModel(cell!!, columnPosition) }
        }
    }
    ...
    override fun getCellItemViewType(position: Int): Int = MenuDishTableViewModel.getCellItemViewType(position)
    ...
    fun setMenuDishList(menus: List<Menu>) {
        menuDishTableViewModel.generateListForTableView(menus)
        setAllItems(
            menuDishTableViewModel.columnHeaderModeList, menuDishTableViewModel
                .rowHeaderModelList, menuDishTableViewModel.cellModelList
        )
    }
}