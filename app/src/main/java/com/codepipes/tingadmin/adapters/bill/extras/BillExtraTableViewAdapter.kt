...
class BillExtraTableViewAdapter (val context: Context) : AbstractTableAdapter<ColumnHeaderModel?, RowHeaderModel?, CellModel?>() {

    private val billExtraTableViewModel: BillExtraTableViewModel = BillExtraTableViewModel()

    override fun onCreateCellViewHolder(parent: ViewGroup, viewType: Int): AbstractViewHolder {
        return when (viewType) {
            Constants.IMAGE_TABLE_VIEW_CELL -> {
                val layout = LayoutInflater.from(context)
                    .inflate(R.layout.tableview_cell_delete_extra_view, parent, false)
    ...
    override fun getCellItemViewType(position: Int): Int =
        BillExtraTableViewModel.getCellItemViewType(position)
    ...
    fun setExtrasList(extras: List<BillExtra>, currency: String) {
        billExtraTableViewModel.generateListForTableView(extras, currency)
        setAllItems(
            billExtraTableViewModel.columnHeaderModeList, billExtraTableViewModel
                .rowHeaderModelList, billExtraTableViewModel.cellModelList
        )
    }
}
