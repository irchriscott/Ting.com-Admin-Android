...
class PromotionTableViewAdapter (val context: Context) : AbstractTableAdapter<ColumnHeaderModel?, RowHeaderModel?, CellModel?>() {

    private val promotionTableViewModel: PromotionTableViewModel = PromotionTableViewModel()

    override fun onCreateCellViewHolder(parent: ViewGroup, viewType: Int): AbstractViewHolder {
        return when (viewType) {
            Constants.PROMOTION_TABLE_VIEW_CELL -> {
                val layout = LayoutInflater.from(context)
                    .inflate(R.layout.tableview_cell_promotion, parent, false)
                PromotionCellViewHolder(layout)
            }
    ...
    override fun onBindCellViewHolder(
        ...
        when (holder) {
            ...
            is PromotionCellViewHolder -> {
                (holder as PromotionCellViewHolder).setCellModel(cell!!, columnPosition)
            }
    ...
    override fun getCellItemViewType(position: Int): Int =
        PromotionTableViewModel.getCellItemViewType(position)
    ...
    fun setPromotionsList(promotions: List<MenuPromotion>) {
        promotionTableViewModel.generateListForTableView(promotions)
        setAllItems(
            promotionTableViewModel.columnHeaderModeList, promotionTableViewModel
                .rowHeaderModelList, promotionTableViewModel.cellModelList
        )
    }
}