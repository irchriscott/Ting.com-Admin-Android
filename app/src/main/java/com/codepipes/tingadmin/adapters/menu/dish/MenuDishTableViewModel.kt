...
class MenuDishTableViewModel {
    ...
    private fun createCellModelList(menus: List<Menu>): List<List<CellModel>> {
        ...
        for (i in menus.indices) {
            ...
            list.add(CellModel("4-menu-$i", menu.category!!.name))
            list.add(CellModel("5-menu-$i", price))
            list.add(CellModel("6-menu-$i", Constants.DISH_TIME[menu.dishTime]!!))
            list.add(CellModel("7-menu-$i", menu.cuisine!!.name))
            list.add(CellModel("8-menu-$i", if(menu.isAvailable) { "YES" } else { "NO" }))
            list.add(CellModel("9-menu-$i", if(menu.hasDrink == true) { "${menu.drink?.images!!.images[0].image}::${menu.drink.name}" } else { "" }))
            list.add(CellModel("10-menu-$i", menu.id.toString()))
    ...
    companion object {
        public fun getCellItemViewType(column: Int): Int {
            return when (column) {
                ...
                8 -> Constants.WAITER_TABLE_VIEW_CELL
                9 -> Constants.ACTIONS_TABLE_VIEW_CELL
