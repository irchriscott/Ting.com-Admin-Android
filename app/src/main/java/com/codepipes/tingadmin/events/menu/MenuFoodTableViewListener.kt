...
class MenuFoodTableViewListener (
            ...
            override fun data(data: String, position: Int) {
                when (data) {
                    ...
                    menuDataList[1] -> {
                        ...
                        val bundle = Bundle()
                        bundle.putString(Constants.CONFIRM_TITLE_KEY, "${menuDataList[2]?.capitalize()} Menu Food")
                        bundle.putString(Constants.CONFIRM_MESSAGE_KEY, "Do you really want to ${menuDataList[2]?.toLowerCase()} this menu food ?")
                        ...
                        confirmDialog.onDialogListener(object : ConfirmDialogListener {
                            override fun onAccept() {
                                ...
                                TingClient.getRequest("${Routes.menusFoodAvailToggle}${menu.id}/", null, session.token) { _, isSuccess, result ->
                    ...
                    menuDataList[2] -> {
                        val confirmDialog = ConfirmDialog()
                        val bundle = Bundle()
                        bundle.putString(Constants.CONFIRM_TITLE_KEY, "Delete Menu Food")
                        bundle.putString(Constants.CONFIRM_MESSAGE_KEY, "Do you really want to delete this menu food ?")
                        confirmDialog.arguments = bundle
                        confirmDialog.onDialogListener(object : ConfirmDialogListener {
                            override fun onAccept() {
                                confirmDialog.dismiss()
                                val progressOverlay = ProgressOverlay()
                                progressOverlay.show(fragmentManager, progressOverlay.tag)
                                TingClient.getRequest("${Routes.menusFoodDelete}${menu.id}/", null, session.token) { _, isSuccess, result ->
                                    activity.runOnUiThread {
                                        progressOverlay.dismiss()
                                        if(isSuccess) {
                                            try {
                    ...
                    menuDataList[3] -> {
                        val types = Constants.FOOD_TYPE
                        val selectDialog = SelectDialog()
                        val selectBundle = Bundle()
                        selectBundle.putString(Constants.CONFIRM_TITLE_KEY, "Select Food Type")
                        selectDialog.arguments = selectBundle
                        selectDialog.setItems(types.toSortedMap().map { it.value }, object : SelectItemListener {
                            override fun onSelectItem(position: Int) {
                                val selectedType = position + 1
                                val confirmDialog = ConfirmDialog()
                                val bundle = Bundle()
                                bundle.putString(Constants.CONFIRM_TITLE_KEY, "Change Menu Food Type")
                                bundle.putString(Constants.CONFIRM_MESSAGE_KEY, "Do you really want to change this menu food's type to ${types[selectedType]} ?")
                                confirmDialog.arguments = bundle
                                confirmDialog.onDialogListener(object : ConfirmDialogListener {
                                    override fun onAccept() {
                                        confirmDialog.dismiss()
                                        ...
                                        TingClient.getRequest("${Routes.menusFoodMoveToType}${menu.id}/to/$selectedType/", null, session.token) { _, isSuccess, result ->
                                            activity.runOnUiThread {
                                                progressOverlay.dismiss()
                                                if(isSuccess) {
                                                    try {
                            ...
                            activity.runOnUiThread {
                                if (isSuccess) {
                                    try {
                                        ...
                                        val selectDialog = SelectDialog()
                                        val selectBundle = Bundle()
                                        selectBundle.putString(Constants.CONFIRM_TITLE_KEY, "Select Food Category")
                                        selectDialog.arguments = selectBundle
                                        selectDialog.setItems(categories.map { it.name }, object : SelectItemListener {
                                            override fun onSelectItem(position: Int) {
                                                val selectedCategory = categories[position]
                                                val confirmDialog = ConfirmDialog()
                                                val bundle = Bundle()
                                                bundle.putString(Constants.CONFIRM_TITLE_KEY, "Change Menu Food Category")
                                                bundle.putString(Constants.CONFIRM_MESSAGE_KEY, "Do you really want to change this menu food's category to ${selectedCategory.name} ?")
                                                confirmDialog.arguments = bundle
                                                confirmDialog.onDialogListener(object : ConfirmDialogListener {
                                                    override fun onAccept() {
                                                        confirmDialog.dismiss()
                                                        ...
                                                        progressOverlay.show(fragmentManager, progressOverlay.tag)
                                                        selectDialog.dismiss()
                                                        TingClient.getRequest("${Routes.menusFoodMoveToCategory}${menu.id}/to/${selectedCategory.id}/", null, session.token) { _, isSuccess, result ->
                                                            activity.runOnUiThread {
                                                                progressOverlay.dismiss()
                                                                if(isSuccess) {
                                                                    try {
                    ...
                    menuDataList[5] -> {
                        ...
                        val selectDialog = SelectDialog()
                        val selectBundle = Bundle()
                        selectBundle.putString(Constants.CONFIRM_TITLE_KEY, "Select Food Cuisine")
                        selectDialog.arguments = selectBundle
                        selectDialog.setItems(cuisines.map { it.name }, object : SelectItemListener {
                            override fun onSelectItem(position: Int) {
                                ...
                                val confirmDialog = ConfirmDialog()
                                val bundle = Bundle()
                                bundle.putString(Constants.CONFIRM_TITLE_KEY, "Change Menu Food Cuisine")
                                bundle.putString(Constants.CONFIRM_MESSAGE_KEY, "Do you really want to change this menu food's cuisine to ${selectedCuisine.name} ?")
                                confirmDialog.arguments = bundle
                                confirmDialog.onDialogListener(object : ConfirmDialogListener {
                                    override fun onAccept() {
                                        confirmDialog.dismiss()
                                        ...
                                        progressOverlay.show(fragmentManager, progressOverlay.tag)
                                        selectDialog.dismiss()
                                        TingClient.getRequest("${Routes.menusFoodMoveToCuisine}${menu.id}/to/${selectedCuisine.id}/", null, session.token) { _, isSuccess, result ->
                                            activity.runOnUiThread {
                                                progressOverlay.dismiss()
                                                if(isSuccess) {
                                                    try {
